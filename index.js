...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oii @${num.split('@')[0]}\Bem vindo(a) ao grupo *${mdata.subject}* Por favor, leia as regras e respeite a todos do grupo e evite ao máximo o seu banimento, sinta-se em casa😗`
				...
				teks = `Finalmente a carga do grupo foi reduzida ,tchau tchau @${num.split('@')[0]} hshshsh até um outro dia `
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                case 'ytmp3':
                    if (!isRegistered) return reply(ind.noregis())
                    if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply('Onde está o url hm? ')
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                 ...
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   const aruga1 = q.substring(0, q.indexOf('|') - 0)
					...
					client.sendMessage(from, aruga, image, {caption: 'Prontinho✅', quoted: mek})
					await limitAdd(sender)
					break 
					case 'summer':
					if (!isRegistered) return reply(ind.noregis())
				...
				case 'bcgc':
                   ...
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Esta é uma lista de grupos :\n\n\`\`\``
					no = 0
                ...
                if (isGroup) {
                    ...
                    console.log(color('[REGISTRO]'), color(time, 'yellow'), 'Nome:', color(namaUser, 'cyan'), 'Era:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'), 'dentro', color(sender || groupName))
                } else {
                    addRegisteredUser(sender, namaUser, umurUser, time, serialUser)
                    await reply(ind.registered(namaUser, umurUser, serialUser, time, sender))
                    addATM(sender)
          ...
          case 'ping':
          if (!isRegistered) return reply(ind.noregis())
            await client.sendMessage(from, `\nSpeed: ${processTime(time, moment())} _Segundos_`)
					break
               case 'help': 
				case 'menu':
				if (!isRegistered) return reply(ind.noregis())
                ...
                resul = `┏━━❉ *LEVEL* ❉━━\n┣⊱ *Nome* : ${pushname}\n┣⊱ Número : wa.me/${sender.split("@")[0]}\n┣⊱ Xp Do utilizador :  ${userXp}/${requiredXp}\n┣⊱ Nível de usuário : ${userLevel}\n┗━━━━━━━━━━━━`
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Use o prefixo ${prefix} para pegar o texto')
					}
					await limitAdd(sender)
					break
				case 'stiker': 
				...
				case 'clearall':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `Pedido recebido, você foi desbloqueado ${body.slice(9)}@c.us`, text)
					break
				case 'sair': 
				if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				await reply(from, 'Adeus!').then(() => client.groupLeave(groupId))
					break
				case 'bc': 
					if (!isOwner) return reply(ind.ownerb()) 
					if (args.length < 1) return reply('.......')
				...
				case 'adicionar':
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar destino, talvez porque o número esteja privado ')
					}
					break
					case 'grup':
					case 'grupo':
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Este é o número do meu proprietário, não envie spam ou bloqueie-me 'MessageType.text, { quoted: mek} )
					break    
           case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
                ...
                client.sendMessage(from, 'Pronto, nome do grupo alterado com sucesso✅', text, {quoted: mek})
					break
                case 'setdesc':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
                ...
                client.sendMessage(from, 'Pronto, descrição do Grupo alterada com sucesso ✅', text, {quoted: mek})
					break
           case 'demote':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
				...
				case 'promote':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque quem você deseja tornar administrador!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Parabéns, você agora é um dos administradores do grupo😙💜 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Parabéns @${mentioned[0].split('@')[0]} *Você se tornou um administrador desse grupo!`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'remover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Removido com suceso✅ @${mentioned[0].split('@')[0]} `, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'toimg':
				if (!isRegistered) return reply(ind.noregis())
				if (!isQuotedSticker) return reply('Marque a figurinha que deseja converter em imagem!')
					reply(ind.wait())
				...
				case 'nsfw':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply(':T')
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply(' *Ativado!*')
						nsfw.push(from)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('Sucesso na ativação✅')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('Sucesso na ativação✅')
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Boo :𝘃')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('*Boas vindas foi ativado!*')
						welkom.push(from)
				...
				case 'clonar':
					...
					if (args.length < 1) return reply(' *MARQUE A PESSOA QUE VOCE DESEJA CLONAR* ')
